...
<script>
import { listModel, getModel, delModel, addModel, updateModel } from "@/api/process/model"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除转辙机型号编号为"' + modelIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('process/model/export', {
        ...this.queryParams
